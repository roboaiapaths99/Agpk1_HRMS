...
from services.exit_management_service import ExitManagementService
from schemas.exit_management_schema import (
    ...
    ExitSearch, ExitTaskCreate, ExitTaskUpdate
)
...
async def get_exit_management(
    ...
    exit_management = await exit_service.get_exit_management_by_id(exit_id)
    return exit_management
...
async def get_exit_by_employee(
    ...
    # Users can view their own exit process
    if current_user.role.value != "admin" and current_user.employee_id != employee_id:
        require_permission(current_user, "exit_management", "read")
    
    exit_management = await exit_service.get_exit_management_by_employee(employee_id)
    return exit_management
...
async def update_exit_management(
    ...
    exit_management = await exit_service.update_exit_management(exit_id, update_data, current_user.username)
    return exit_management
...
async def add_task_to_exit(
    ...
    exit_management = await exit_service.add_task_to_exit(exit_id, task_data, current_user.username)
    return exit_management
...
async def complete_task(
    ...
    exit_management = await exit_service.complete_task(exit_id, task_id, current_user.username, notes)
    return exit_management
...
async def update_task_status(
    ...
    exit_management = await exit_service.update_task_status(exit_id, task_id, task_update, current_user.username)
    return exit_management
...
async def complete_exit(
    ...
    exit_management = await exit_service.complete_exit(exit_id, parsed_date, current_user.username)
    return exit_management
...
async def conduct_exit_interview(
    ...
    exit_management = await exit_service.conduct_exit_interview(exit_id, interview_data, current_user.username)
    return exit_management


@router.post("/{exit_id}/settlement/calculate", response_model=ExitManagementResponse)
async def calculate_final_settlement(
    ...
    exit_management = await exit_service.calculate_final_settlement(exit_id, current_user.username)
    return exit_management
...
async def process_final_settlement(
    ...
    exit_management = await exit_service.process_final_settlement(exit_id, settlement_data, current_user.username)
    return exit_management
...
async def get_my_exit_process(
    ...
    exit_management = await exit_service.get_exit_management_by_employee(current_user.employee_id)
    return exit_management
...
async def get_compliance_report(
    ...
    report = await exit_service.get_compliance_report(payroll_month)
    return report
