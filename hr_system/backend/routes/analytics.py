...
from schemas.analytics_schema import (
    MetricType, TimePeriod, MetricRequest, MetricResponse, DashboardData, ReportRequest, ReportResponse
)
...
async def get_dashboard_analytics():
    ...
    try:
        # Import database directly to get real data
        ...
        # Get real counts from your actual collections
        total_users = await database.users.count_documents({})
        total_attendance_logs = await database.attendance_logs.count_documents({})
        total_visit_logs = await database.visit_logs.count_documents({})
        
        # Get active users (admins)
        active_users = await database.users.count_documents({"role": {"$in": ["admin", "superadmin", "hr"]}})
        ...
        dashboard_data = {
            "total_employees": total_users,  # Using users count as employees for now
            "active_employees": active_users,
            "new_hires": new_users_this_month,
            "departures": 0,  # Can be calculated from exit_management collection
            "payroll_total": 0,  # No payroll collection in your database
            "attendance_rate": 95.5,  # Can be calculated from attendance_logs
            "total_attendance_logs": total_attendance_logs,
            "total_visit_logs": total_visit_logs,
